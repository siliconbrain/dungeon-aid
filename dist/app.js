...
exports.races = exports.classes = undefined;
...
var Die = function Die(sides) {
    var die = function die() {
        return Math.floor(Math.random() * sides + 1.0);
    };
    die.sides = sides;
    die.maxValue = sides;
    return die;
 ...
 */
var getAbilityModifier = function getAbilityModifier(abilityScore) {
    return Math.floor(Math.max(1, abilityScore) / 2) - 5;
};
var getBonusSpells = function getBonusSpells(abilityScore, spellLevel) {
    return spellLevel == 0 ? null : Math.max(0, Math.floor((getAbilityModifier(abilityScore) - spellLevel) / 4) + 1);
};

var rollAbilityScore = function rollAbilityScore() {
    return new Array(4).fill(d6).map(function (die) {
        return die();
    }).sort().slice(-3).reduce(_util.sum);
};
var areAbilityScoresTooLow = function areAbilityScoresTooLow(scores) {
    return scores.slice().sort()[5] <= 13 || scores.map(getAbilityModifier).reduce(_util.sum);
}; // scores: Base scores without racial adjustments.
...
var getLevel = function getLevel(experience) {
    return Math.floor((-1 + Math.sqrt(1 + 4 * experience / 500)) / 2) + 1;
};

var levelUpAt = function levelUpAt(currentLevel) {
    return 500 * currentLevel * (currentLevel + 1);
};

function makeCharacter(_ref) {
    var _ref$abilities = _ref.abilities;
    var strengthBase = _ref$abilities.strengthBase;
    var dexterityBase = _ref$abilities.dexterityBase;
    var constitutionBase = _ref$abilities.constitutionBase;
    var intelligenceBase = _ref$abilities.intelligenceBase;
    var wisdomBase = _ref$abilities.wisdomBase;
    var charismaBase = _ref$abilities.charismaBase;
    var age = _ref.age;
    var alignment = _ref.alignment;
    var classes_ = _ref.classes;
    var experience = _ref.experience;
    var feats = _ref.feats;
    ...
    function makeAbility(base, racialAdjustment) {
        return {
            base: base,
            get score() {
                return this.base + racialAdjustment;
            },
            get mod() {
    ...
    var armorBonus = 0; // TODO: calc
    var shieldBonus = 0; // TODO: calc
    var sizeModifier = 0; // TODO: calc
    var improvedInitiativeFeatModifier = 0; // TODO: calc
    var baseAttackBonus = 0; // TODO: calc
    classes_ = classes_.reduce(function (accu, elem) {
        return classes.hasOwnProperty(elem) ? accu.concat(elem) : accu;
    }, []);
    return {
        abilities: abilities,
        age: age,
...
},{"./util.es6":3}],2:[function(require,module,exports){
"use strict";
...
var _character = require("./character.es6");
...
function loadState(storage) {
    ...
    return storage.local.getItem("character").map(JSON.parse).map(function (data) {
        return data === null ? defaultState : {
            character: {
                abilities: {
...
function toPresentable(_ref) {
    var storedState$ = _ref.storedState$;
    var action$ = _ref.action$;
    // TODO: better function name?
    return _rx2.default.Observable.combineLatest(storedState$, action$.filter(function (action) {
        return !action.verb.startsWith("set");
    }).startWith({}), function (storedState, action) {
        return {
            editingName: action.verb == "editName",
            editingRace: action.verb == "editRace",
            editingClass: action.verb == "editClass",
            character: (0, _character.makeCharacter)(storedState.character)
        };
    });
}

function presentAbility(ability, name) {
    return (0, _dom.div)(".ability." + name.toLowerCase(), [(0, _dom.div)([(0, _dom.span)(".score", String(ability.score)), (0, _dom.span)(".modifier" + (ability.mod > 0 ? ".bonus" : ability.mod < 0 ? ".penalty" : ""), String(Math.abs(ability.mod))), (0, _dom.input)(".base", { type: "number", min: "3", max: "18", value: String(ability.base) })]), (0, _dom.label)(name)]);
}

function presentCharacterName(character, editing) {
    return (0, _dom.span)(".character.name", editing ? (0, _dom.input)(".request-focus", { type: "text", value: character.name }) : [(0, _dom.span)(".text" + (character.name ? "" : ".unknown"), character.name), (0, _dom.span)(".edit.fa.fa-pencil")]);
}

function presentRaceSelector(character) {
    var options = [(0, _dom.option)({ value: "" }, "---")];
    for (var race in _character.races) {
        if (_character.races.hasOwnProperty(race)) {
            options.push((0, _dom.option)(character.race == race ? { selected: true } : {}, race));
        }
    }
    return (0, _dom.select)(".request-focus", options);
}

function presentCharacterRace(character, editing) {
    return (0, _dom.span)(".character.race", editing ? presentRaceSelector(character) : [(0, _dom.span)(".text" + (character.race ? "" : ".unknown"), character.race), (0, _dom.span)(".edit.fa.fa-pencil")]);
}

function presentClassSelector(character) {
    var primaryClass = character.classes[0];
    var options = [(0, _dom.option)({ value: "" }, "---")];
    for (var class_ in _character.classes) {
        if (_character.classes.hasOwnProperty(class_)) {
            options.push((0, _dom.option)(primaryClass == class_ ? { selected: true, value: class_ } : { value: class_ }, class_));
        }
    }
    return (0, _dom.select)(".request-focus", options);
}

function presentCharacterClass(character) {
    var editing = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];

    return (0, _dom.span)(".character.class", editing ? presentClassSelector(character) : [(0, _dom.span)(".text" + (character.classes[0] ? "" : ".unknown"), character.classes[0]), (0, _dom.span)(".edit.fa.fa-pencil")]);
}
...
function observe(vtree) {
    var editName$ = vtree.select(".character.name").select(".edit").events("click").map(function (ev) {
        return { verb: "editName" };
    });
    var cancelNameEditing$ = vtree.select(".character.name").select("input").events("blur").map(function (ev) {
        return { verb: "cancelNameEditing" };
    });
    var changeName$ = vtree.select(".character.name").select("input").events("change").map(function (ev) {
        return { verb: "setName", value: ev.target.value.trim() };
    });
    var editRace$ = vtree.select(".character.race").select(".edit").events("click").map(function (ev) {
        return { verb: "editRace" };
    });
    var cancelRaceEditing$ = vtree.select(".character.race").select("select").events("blur").map(function (ev) {
        return { verb: "cancelRaceEditing" };
    });
    var changeRace$ = vtree.select(".character.race").select("select").events("change").map(function (ev) {
        return { verb: "setRace", value: ev.target.value };
    });
    var editClass$ = vtree.select(".character.class").select(".edit").events("click").map(function (ev) {
        return { verb: "editClass" };
    });
    var cancelClassEditing$ = vtree.select(".character.class").select("select").events("blur").map(function (ev) {
        return { verb: "cancelClassEditing" };
    });
    var changeClass$ = vtree.select(".character.class").select("select").events("change").map(function (ev) {
        return { verb: "setClass", value: ev.target.value };
    });
    var changeExperience$ = vtree.select(".character.experience").select("input").events("change").map(function (ev) {
        return { verb: "setExperience", value: Number(ev.target.value) };
    });
    var changeStrength$ = vtree.select(".strength").select(".base").events("change").map(function (ev) {
        return { verb: "setStrength", value: Number(ev.target.value) };
    });
    var changeDexterity$ = vtree.select(".dexterity").select(".base").events("change").map(function (ev) {
        return { verb: "setDexterity", value: Number(ev.target.value) };
    });
    var changeConstitution$ = vtree.select(".constitution").select(".base").events("change").map(function (ev) {
        return { verb: "setConstitution", value: Number(ev.target.value) };
    });
    var changeIntelligence$ = vtree.select(".intelligence").select(".base").events("change").map(function (ev) {
        return { verb: "setIntelligence", value: Number(ev.target.value) };
    });
    var changeWisdom$ = vtree.select(".wisdom").select(".base").events("change").map(function (ev) {
        return { verb: "setWisdom", value: Number(ev.target.value) };
    });
    var changeCharisma$ = vtree.select(".charisma").select(".base").events("change").map(function (ev) {
        return { verb: "setCharisma", value: Number(ev.target.value) };
    });
    var exportCharacter$ = vtree.select(".export").events("click").map(function (ev) {
        return { verb: "exportCharacter" };
    });
    return _rx2.default.Observable.merge(editName$, cancelNameEditing$, changeName$, editRace$, cancelRaceEditing$, changeRace$, editClass$, cancelClassEditing$, changeClass$, changeExperience$, changeStrength$, changeDexterity$, changeConstitution$, changeIntelligence$, changeWisdom$, changeCharisma$, exportCharacter$).do(function (action) {
        return console.log(action);
    });
}

function toPersistable(_ref2) {
    var storedState$ = _ref2.storedState$;
    var action$ = _ref2.action$;
    // TODO: better function name
    return action$.withLatestFrom(storedState$, function (action, storedState) {
        return {
            character: {
                abilities: {
...
function saveState(state$) {
    ...
    }).map(JSON.stringify).map(function (value) {
        return {
            key: "character",
            target: "local",
            value: value
...
function main(_ref3) {
    var vtree = _ref3.vtree;
    var storage = _ref3.storage;

    var storedState$ = loadState(storage);
    var action$ = observe(vtree).share();

    return {
        vtree: present(toPresentable({ storedState$: storedState$, action$: action$ })),
        storage: saveState(toPersistable({ storedState$: storedState$, action$: action$ })),
        exporter: action$.filter(function (action) {
            return action.verb == "exportCharacter";
        }).map(function () {
            return { data: localStorage.character, filename: "character.json" };
        }),
...
_core2.default.run(main, {
    vtree: (0, _dom.makeDOMDriver)("#app"),
    storage: _storage2.default,
    exporter: function exporter(request$) {
        request$.subscribe(function (request) {
            return saveAs(new Blob([request.data], { type: request.type || "text/plain;charset=utf-8" }), request.filename);
        });
        return _rx2.default.Observable.never(); // write-only driver
    },
    focuser: function focuser(vtree$) {
        // give focus where focus is due
        vtree$.subscribe(function () {
            var element = document.getElementsByClassName("request-focus")[0];
            if (element) {
                element.focus();
...
},{"./character.es6":1,"@cycle/core":4,"@cycle/dom":5,"@cycle/isolate":13,"@cycle/storage":14,"rx":31}],3:[function(require,module,exports){
"use strict";
...
exports.sum = sum;
function sum() {
    return arguments.reduce(function (accu, elem) {
        return accu + elem;
    }, 0);
}
...
},{"rx":31}],5:[function(require,module,exports){
"use strict";
 ...
 */
},{"./mock-dom-source":7,"./render-dom":8,"./render-html":9,"./virtual-hyperscript":12,"./virtual-hyperscript-svg":11,"hyperscript-helpers":24}],6:[function(require,module,exports){
"use strict";
...
},{"rx":31}],7:[function(require,module,exports){
'use strict';
...
},{"rx":31}],8:[function(require,module,exports){
"use strict";
...
},{"./fromevent":6,"./transposition":10,"./virtual-hyperscript":12,"matches-selector":29,"rx":31,"vdom-parser":36,"virtual-dom/diff":43,"virtual-dom/patch":44}],9:[function(require,module,exports){
"use strict";
...
},{"./transposition":10,"rx":31,"vdom-to-html":40}],10:[function(require,module,exports){
"use strict";
...
},{"rx":31,"virtual-dom/vnode/vnode":63}],11:[function(require,module,exports){
/*eslint-disable */
'use strict';
...
// END Cycle.js-specific code <<<<<<<<<<
},{"./virtual-hyperscript":12,"virtual-dom/virtual-hyperscript/hooks/attribute-hook":51,"virtual-dom/virtual-hyperscript/svg-attribute-namespace":55,"x-is-array":68}],12:[function(require,module,exports){
/* eslint-disable */
'use strict';
...
/* eslint-enable */
},{"virtual-dom/virtual-hyperscript/hooks/ev-hook.js":52,"virtual-dom/virtual-hyperscript/hooks/soft-set-hook.js":53,"virtual-dom/virtual-hyperscript/parse-tag.js":54,"virtual-dom/vnode/is-thunk":57,"virtual-dom/vnode/is-vhook":58,"virtual-dom/vnode/is-vnode":59,"virtual-dom/vnode/is-vtext":60,"virtual-dom/vnode/is-widget":61,"virtual-dom/vnode/vnode.js":63,"virtual-dom/vnode/vtext.js":65,"x-is-array":68}],13:[function(require,module,exports){
"use strict";
...
},{"./responseCollection":15,"./writeToStore":17}],15:[function(require,module,exports){
'use strict';
...
},{"rx":31}],17:[function(require,module,exports){
"use strict";
...
},{"individual/one-version":26}],23:[function(require,module,exports){
(function (global){
...
},{"min-document":18}],24:[function(require,module,exports){
'use strict';
...
},{"sentence-case":32}],31:[function(require,module,exports){
...
},{"./vendor/camel-case-regexp":33,"./vendor/non-word-regexp":34,"./vendor/trailing-digit-regexp":35,"lower-case":28}],33:[function(require,module,exports){
...
},{"./namespace-map":37,"./property-map":38,"virtual-dom/vnode/vnode":63,"virtual-dom/vnode/vtext":65}],37:[function(require,module,exports){
...
},{"./property-config":41,"escape-html":21}],40:[function(require,module,exports){
var escape = require('escape-html');
...
},{"./create-attribute":39,"./void-elements":42,"escape-html":21,"param-case":30,"virtual-dom/virtual-hyperscript/hooks/attribute-hook":51,"virtual-dom/virtual-hyperscript/hooks/soft-set-hook":53,"virtual-dom/vnode/is-thunk":57,"virtual-dom/vnode/is-vnode":59,"virtual-dom/vnode/is-vtext":60,"virtual-dom/vnode/is-widget":61,"xtend":69}],41:[function(require,module,exports){
/**
 * Attribute types.
 */
...
},{"../vnode/is-vhook.js":58,"is-object":27}],46:[function(require,module,exports){
...
},{"../vnode/handle-thunk.js":56,"../vnode/is-vnode.js":59,"../vnode/is-vtext.js":60,"../vnode/is-widget.js":61,"./apply-properties":45,"global/document":23}],47:[function(require,module,exports){
...
},{"../vnode/is-widget.js":61,"../vnode/vpatch.js":64,"./apply-properties":45,"./update-widget":50}],49:[function(require,module,exports){
var document = require("global/document")
var isArray = require("x-is-array")
...
},{"./create-element":46,"./dom-index":47,"./patch-op":48,"global/document":23,"x-is-array":68}],50:[function(require,module,exports){
var isWidget = require("../vnode/is-widget.js")
...
},{"../vnode/is-widget.js":61}],51:[function(require,module,exports){
'use strict';
...
},{"browser-split":19}],55:[function(require,module,exports){
'use strict';
...
},{"../vnode/is-vhook":58,"is-object":27}],67:[function(require,module,exports){
var isArray = require("x-is-array")
